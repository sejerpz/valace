		...
		public override void create_control ()
		{
			window = Window.create (
        		...
        		if (window == Window.NULL) {
        			MessageBox.error ("Create container failed");
			}
		}
		...
		public override int process_message (Window win, uint msg_id, WParam w_param, LParam l_param, ref bool processed)
		{
			...
			switch (msg_id) {
				case MessageId.COMMAND:
					var button = find_from_window ((Window) l_param);
					if (button != null) {
						result = button.process_message (window, msg_id, w_param, l_param, ref processed );	
					}
					break;
				default:
