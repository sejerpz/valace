...
#define win32_check_for_error(what) G_STMT_START{					\
  if (!(what)) {									\
	gchar *msg =  g_win32_error_message (GetLastError ());				\
        int len = MultiByteToWideChar ((guint) 65001, (DWORD) 0, msg, -1, NULL, 0); 	\
        gchar* buff = g_new0 (gchar, len); 						\
        MultiByteToWideChar ((guint) 65001, (DWORD) 0, msg, -1, buff, len); 		\
              MessageBox (NULL, buff, NULL,						\
...
g_thread_self_win32_impl (gpointer thread)
{
  ...
  if (!self)
	...
	self->thread = handle; // this is not correct, but I haven't found any DuplicateHandle equivalent
#else
